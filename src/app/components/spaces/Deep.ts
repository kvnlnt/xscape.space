...
type DeepEvents = 'SLIDE_IN' | 'SLIDE_OUT' | 'ACTIVATE' | 'DEACTIVATE' | 'ON_ACTIVATION';
type DeepSpaceProps = {
  onActivation?: () => void;
  onDeactivation?: () => void;
};

export const useDeepSpace = ({
  onActivation,
  onDeactivation,
}: DeepSpaceProps): [HTMLElement, (event: DeepEvents) => void] => {
  // props
  ...
  // state machine
  const machine = (event: DeepEvents) => {
