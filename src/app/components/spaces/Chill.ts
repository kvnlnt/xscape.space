...
type ChillEvents = 'SLIDE_IN' | 'SLIDE_OUT' | 'ACTIVATE' | 'DEACTIVATE' | 'ON_ACTIVATION';
type ChillSpaceProps = {
  onActivation?: () => void;
  onDeactivation?: () => void;
};
...
export const useChillSpace = ({
  onActivation,
  onDeactivation,
}: ChillSpaceProps): [HTMLElement, (event: ChillEvents) => void] => {
  // props
  ...
  // state machine
  const machine = (event: ChillEvents) => {
